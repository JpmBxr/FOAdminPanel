          ...
          <v-list-item two-line>
            <v-list-item-content>
              <v-list-item-title class="text-h5">
                <strong> {{ this.$t("label_assignment") }}</strong>
              </v-list-item-title>
        ...
        >
          ...
          >
            ...
            <v-icon
              ...
              @click="
                expand(true);
                getDocumentAssignmentWise(item.lms_assignment_id);
              "
              v-if="!isExpanded"
              size="22"
              class="mr-2"
          ...
          <template v-slot:expanded-item="{ headers, item }">
            <td :colspan="headers.length" style="bgcolor: red">
              ...
              <v-row dense class="mx-2 my-2">
                <v-col cols="12" md="6">
                  <v-card :disabled="isSourceDataLoading" elevation="2">
                    <v-toolbar color="light-blue" dark>
                      <v-btn icon>
                        <v-icon>mdi-file-document-multiple-outline</v-icon>
                      </v-btn>
                      <v-toolbar-title>Assignment(s)</v-toolbar-title>
                    ...
                    <v-card-text>
                      <v-form
                        ...
                        v-model="isUploadAssignmentFormValid"
                        lazy-validation
                      >
                        <v-row>
                          <v-file-input
                            class="px-4"
                            v-model="selectedAssignmentImage"
                            color="primary"
                            outlined
                            dense
                            show-size
                    ...
                    <v-list subheader two-line>
                      <v-list-item
                        ...
                        :key="folder.lms_assignment_document_id"
                      >
                        <v-list-item-avatar>
                          <v-icon class="info" dark> mdi-file-document </v-icon>
                        </v-list-item-avatar>

                        <v-list-item-content>
                          <v-list-item-title
                            v-text="folder.lms_assignment_document_path"
                          ></v-list-item-title>

                          <v-list-item-subtitle
                            v-text="folder.lms_assignment_document_created_at"
                          ></v-list-item-subtitle>
                        </v-list-item-content>
                ...
                <v-col cols="12" md="6">
                  <v-card :disabled="isSourceDataLoading" elevation="2">
                    <v-toolbar color="light-blue" dark>
                      ...
                      <v-toolbar-title>Assignment Description</v-toolbar-title>

                      <v-spacer></v-spacer>
                    </v-toolbar>
                    <div
                      class="px-2 pt-2"
                      v-html="item.lms_assignment_description"
          ...
          <template v-slot:item.lms_assignment_upload_status="{ item }">
            <v-chip
              x-small
              :color="getStatusColor(item.lms_assignment_upload_status)"
              dark
...
export default {
  ...
  methods: {
    deleteAssignment(lms_assignment_document_id) {
      let userDecision = confirm("Are you sure you want to delete Assignment");
      if (userDecision) {
        ...
        this.$http
          .post(
            ...
            {
              lms_assignment_document_id: lms_assignment_document_id,
          ...
          .then(({ data }) => {
            ...
            if (
              ...
              if (data.responseData == 1) {
                ...
                this.changeSnackBarMessage("Assignment deleted!!!");
                this.getDocumentAssignmentWise(this.selectedAssignmentId);
              }
      ...
      if (this.$refs.uploadAssignmentForm.validate()) {
        ...
        let postData = new FormData();
        {
          if (this.selectedAssignmentImage != null) {
            postData.append("assignmentImage", this.selectedAssignmentImage);
          }

          postData.append("centerId", ls.get("loggedUserCenterId"));
          postData.append("loggedUserId", ls.get("loggedUserId"));
          postData.append("lms_assignment_id", this.selectedAssignmentId);

          this.$http
            .post("web_upload_assignment", postData, this.authorizationConfig)
            .then(({ data }) => {
              ...
              if (
                ...
                // Course Code Exists
                  ...
                  this.snackBarColor = "success";
                  this.changeSnackBarMessage("Assignment Uploaded");
                  this.getDocumentAssignmentWise(this.selectedAssignmentId);
                  this.selectedAssignmentImage = null;
                }
                // Course Saved
                  ...
                  this.selectedAssignmentImage = null;
                  this.snackBarColor = "error";
                  this.changeSnackBarMessage("Assignment not saved");
                }
                // Failed to save Course
                else if (data.responseData == 3) {
                  this.snackBarColor = "error";
                  this.changeSnackBarMessage("Assignment upload failed");
                }
              }
            })
            .catch((error) => {
    ...
    disableAssignment(item, $event) {
      let userDecision = confirm("Are you sure you want to change status");
      if (userDecision) {
        this.isLoaderActive = true;
        this.$http
          .post(
            "web_enable_disable_assignment",
            {
              ...
              lms_assignment_upload_status:
                item.lms_assignment_upload_status == "Active" ? 0 : 1,
              loggedUserId: ls.get("loggedUserId"),
            },
            this.authorizationConfig
          )
          .then(({ data }) => {
            ...
            if (
              ...
              if (data.responseData == 1) {
                this.snackBarColor = "success";
                this.changeSnackBarMessage("Assignment status changed!!!");
    ...
    saveNotice($event) {
      if (this.$refs.saveAssignmentForm.validate()) {
        ...
        if (this.isSaveEditClicked == 1) {
          postData.append("lms_assignment_title", this.lms_assignment_title);
          postData.append(
          ...
          postData.append("centerId", ls.get("loggedUserCenterId"));
          postData.append("loggedUserId", ls.get("loggedUserId"));
          postData.append("lms_assignment_score", this.lms_assignment_score);
          postData.append(
          ...
          postData.append("selectedCourseId", this.lms_course_id);
          postData.append("lms_batch_id", this.lms_batch_id);
          postData.append("lms_subject_id", this.lms_subject_id);
          postData.append("lms_topic_id", this.lms_topic_id);
          ...
          postData.append("centerId", ls.get("loggedUserCenterId"));
          postData.append("loggedUserId", ls.get("loggedUserId"));
          postData.append("lms_assignment_id", this.lms_assignment_id);
          postData.append("lms_assignment_score", this.lms_assignment_score);
          postData.append(
            "lms_assignment_submission_last_date",
            moment(new Date(this.selectedLiveTill)).format(
          ...
          .then(({ data }) => {
            ...
            if (
              ...
              // Subject Updated
                ...
                this.snackBarColor = "success";
                this.changeSnackBarMessage("Assignment Updated");
                this.getAllActiveAssignment(event);
                this.resetForm();
                this.isSaveEditClicked = 0;
                this.lms_notice_title = "";
